function flightMiddleware(req, res, next) {
  ...
  if (
    ...
    !departureAirportId.match(/^[A-Z]{3}$/)
  ) {
    return res.status(400).json({
      success: false,
      message:
        "departureAirportId is required and must be a 3-letter uppercase airport code.",
    });
  }
  if (
    !arrivalAirportId ||
    typeof arrivalAirportId !== "string" ||
    !arrivalAirportId.match(/^[A-Z]{3}$/)
  ) {
    return res.status(400).json({
      success: false,
      message:
        "arrivalAirportId is required and must be a 3-letter uppercase airport code.",
    });
  }
